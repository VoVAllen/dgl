...
def start_server(num_clients, ip_config):
    ...
    dgl.distributed.start_server(server_id=int(os.getenv("SID", "0")),
                                 ip_config=ip_config,
                                 num_servers=1,
...
def test_multi_client():
    ...
    pserver = ctx.Process(target=start_server, args=(
        10, "rpc_ip_config_mul_client.txt"))
    pclient_list = []
    for i in range(10):
        pclient = ctx.Process(target=start_client, args=(
            "rpc_ip_config_mul_client.txt",))
...
if __name__ == '__main__':
    ...
    import os
    os.environ['DGL_DIST_MODE'] = 'distributed'
    start_server(2, "rpc_ip_config.txt")
    # test_multi_client()
